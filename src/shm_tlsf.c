...
#define B2O(t, x, o) (t *) ((char *) (x) + (o))
#define O2B(t, x, o) (t *) ((char *) (x) - (o))

//#define USE_DEBUG_PRINT
#ifdef USE_DEBUG_PRINT
#define P(fmt, ...) printf("TLSF_DEBUG %s:%d "fmt, __FUNCTION__, __LINE__, ##__VA_ARGS__);
#define PB2O(x, o) P("%s: %p / %p\n", #x, x, B2O(void, x, o))
#define PO2B(x, o) P("%s: %p / %p\n", #x, O2B(void, x, o), x)
#define PX(x) P("%s: 0x%x\n", #x, (int)x)
#define PP(x) P("%s: %p\n", #x, x)
#else
#define P(fmt, ...)
#define PB2O(x, o)
...
#define TLSF_POOL_OVERHEAD(size, num) \
	(((num+8) * size) \
	+ (2*sizeof(void *)) \
	+ sizeof(tlsf_t) \
	+ (sizeof(bhdr_t) * 8) \
...
//#define TLSF_POOL_OVERHEAD(size, num) (size * num  + 65536)
...
/******************************************************************/
    ...
    tlsf->max_size = tlsf->used_size;
#endif
    P("TLSF POOL INIT req %d rem %d\n", (int)mem_pool_size, (int)(b->size & BLOCK_SIZE)); fflush(stdout);
    return (b->size & BLOCK_SIZE);
}
...
/******************************************************************/
    ...
    return offset_malloc_ex(size, mem_pool, 0);
...
static void  _offset_extract_block(ptrdiff_t offset, bhdr_t *_b, tlsf_t *_tlsf, int _fl, int _sl)
{
	// _tlsf = shm
	// _b = shm
	// tmp = base -> shm (+offset)
	// tmp_b = shm -> base (-offset)

	struct bhdr_struct *tmp, **matrix, *tmp_b;

    P("============= EXTRACT BLOCK START =================\n");

    PX(_fl);
    PX(_sl);
	PP(_tlsf -> matrix [_fl][_sl]);
	PO2B(_b, offset);
	PP(_b -> ptr.free_ptr.next);
	PP(_b -> ptr.free_ptr.prev);

	matrix = &_tlsf -> matrix [_fl][_sl];
	tmp_b = O2B(struct bhdr_struct, _b, offset);

	if (_b -> ptr.free_ptr.next) {
		P("NEXT\n");
		tmp = B2O(struct bhdr_struct, _b -> ptr.free_ptr.next, offset);
		tmp -> ptr.free_ptr.prev = _b -> ptr.free_ptr.prev;
		PP(tmp -> ptr.free_ptr.prev);
	}

	if (_b -> ptr.free_ptr.prev){
		P("PREV\n");
		tmp = B2O(struct bhdr_struct, _b -> ptr.free_ptr.prev, offset);
		tmp -> ptr.free_ptr.next = _b -> ptr.free_ptr.next;
		PP(tmp -> ptr.free_ptr.next);
	}

	if (*matrix == tmp_b) {
		P("MATRIX EQUAL b\n");
		*matrix = _b -> ptr.free_ptr.next;
		PP(*matrix);
		if (!*matrix) {
			P("NULL MATRIX");
			clear_bit (_sl, &_tlsf -> sl_bitmap[_fl]);
...
static void _offset_insert_block(ptrdiff_t offset, bhdr_t *_b, tlsf_t *_tlsf, int _fl, int _sl)
{
	// _tlsf = shm
	// _b = shm
	// tmp1  = base
	// tmp_b = shm -> base (-offset)
	// tmp2 = base -> shm (+offset)
	struct bhdr_struct **matrix, *tmp2, *tmp_b;

    P("============= INSERT BLOCK START =================\n");
	...
	_b -> ptr.free_ptr.prev = NULL;
	_b -> ptr.free_ptr.next = *matrix;
	if (*matrix){
		P("MATRIX NOT NULL\n");
		tmp2 = B2O(struct bhdr_struct, *matrix, offset);
		tmp2 -> ptr.free_ptr.prev = tmp_b;
		PP(tmp2 -> ptr.free_ptr.prev);
	}
	*matrix = tmp_b;
	set_bit (_sl, &_tlsf -> sl_bitmap [_fl]);
	set_bit (_fl, &_tlsf -> fl_bitmap);
...
static void _offset_extract_block_hdr(ptrdiff_t offset, bhdr_t *_b, tlsf_t *_tlsf, int _fl, int _sl)
{
	bhdr_t *tmp_b;

    P("============= EXTRACT BLOCK HDR START =================\n");
	...
	_tlsf -> matrix [_fl] [_sl] = _b -> ptr.free_ptr.next;
	if (_tlsf -> matrix[_fl][_sl]){
		tmp_b = B2O(bhdr_t, _tlsf -> matrix[_fl][_sl], offset);
		tmp_b -> ptr.free_ptr.prev = NULL;
		PO2B(tmp_b, offset);
		PP(&tmp_b -> ptr.free_ptr.prev)
	}
	else {
		clear_bit (_sl, &_tlsf -> sl_bitmap [_fl]);
		if (!_tlsf -> sl_bitmap [_fl])
	...
	PP(_tlsf -> matrix [_fl] [_sl]);
	P("============= EXTRACT BLOCK HDR END =================\n\n");
}

/******************************************************************/
void *offset_malloc_ex(size_t size, void *mem_pool, ptrdiff_t offset)
{
/******************************************************************/
    bhdr_t *b, *b2, *next_b;
    int fl, sl;
    ...
    tlsf_t *tlsf = B2O(tlsf_t, mem_pool, offset);
    ...
    b = B2O(bhdr_t, b, offset);
    _offset_extract_block_hdr(offset, b, tlsf, fl, sl);
    ...
    if (tmp_size >= sizeof(bhdr_t)) {
        ...
        b2 = GET_NEXT_BLOCK(b->ptr.buffer, size);
        b2->size = tmp_size | FREE_BLOCK | PREV_USED;
        next_b->prev_hdr = O2B(struct bhdr_struct, b2, offset);
        MAPPING_INSERT(tmp_size, &fl, &sl);
        _offset_insert_block(offset, b2, tlsf, fl, sl);
...
/******************************************************************/
void offset_free_ex(void *ptr, void *mem_pool, ptrdiff_t offset)
{
/******************************************************************/
/*
 * base -> shm = +offset
 ...
 */
    ...
    P("\n============= FREE START =================\n");
    PX(offset);
    PB2O(mem_pool, offset);
    PO2B(b, offset);
    PO2B(tmp_b, offset);
    PX(b->size);

    if (tmp_b->size & FREE_BLOCK) {
    	P("tmp_b->size & FREE_BLOCK\n");
        MAPPING_INSERT(tmp_b->size & BLOCK_SIZE, &fl, &sl);
        PX(fl);
        PX(sl);
        _offset_extract_block(offset, tmp_b, tlsf, fl, sl);
        b->size += (tmp_b->size & BLOCK_SIZE) + BHDR_OVERHEAD;
        PX(b->size);
    }
    if (b->size & PREV_FREE) {
    	P("tmp_b->size & PREV_FREE\n");
    	PP(b->prev_hdr);
        tmp_b = B2O(bhdr_t, b->prev_hdr, offset);
        PO2B(tmp_b, offset);
        MAPPING_INSERT(tmp_b->size & BLOCK_SIZE, &fl, &sl);
        PX(fl);
        PX(sl);
    ...
    PX(fl);
    PX(sl);
    _offset_insert_block(offset, b, tlsf, fl, sl);

    tmp_b = GET_NEXT_BLOCK(b->ptr.buffer, b->size & BLOCK_SIZE);
    tmp_b->size |= PREV_FREE;
    tmp_b->prev_hdr = O2B(struct bhdr_struct, b, offset);
...
size_t tlsf_pool_malloc_size(int num, size_t size)
{
	if (!size || !num){
		return 0;
	}
	return TLSF_POOL_OVERHEAD(size + sizeof(bhdr_t), num);
}

size_t tlsf_pool_create(size_t size, void *ptr)
{
	if (size < tlsf_pool_malloc_size(1, 1)){
		return -1;
	}
	memset(ptr, 0, sizeof(tlsf_t));
